...
class StateCVRPTW(NamedTuple):
    ...
    def update(self, selected):
        ...
        # )[:, 0, :]
        cur_distance = (cur_coord - self.cur_coord).norm(p=2, dim=-1)
        lengths = self.lengths + cur_distance  # (batch_dim, 1)
        # TODO change this later
        # Currently we say that time = distance / 22 
        cur_t = torch.max(self.cur_t + cur_distance / 22, self.arrival_t[self.ids, selected-1] * (selected != 0).float())
        release_t = self.release_t.scatter(-1, selected, cur_t)
